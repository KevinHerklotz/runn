...
export default class PackageJsonParser {
    ...
    setPackageJson(packageJsonOrDirectory) {
        ...
        // If it is a string(directory) or empty we have to calculate the path and get the package.json from there
            ...
            if (!packageJsonOrDirectory) {
                ...
                // TODO: refactor: https://nodejs.org/api/path.html
                if (modulePath.basename(packageJsonOrDirectory) === 'package.json') {
                    packageJsonOrDirectory = packageJsonOrDirectory.slice(0, -12); // eslint-disable-line no-param-reassign
                }

                if (modulePath.isAbsolute(packageJsonOrDirectory)) {
                    directoryOfPackageJson = packageJsonOrDirectory;
                } else {
                    directoryOfPackageJson = modulePath.join(process.cwd(), packageJsonOrDirectory);
                }
            }

            try {
                this.packageJson = require(modulePath.join(directoryOfPackageJson, 'package.json')); // eslint-disable-line global-require
            } catch (err) {
    ...
    concatenateNpmScripts(scriptsObject = this.packageJson.scripts) {
        ...
        // TODO: \s+
        // TODO: https://docs.npmjs.com/cli/run-script  (npm run test -- --grep="pattern")
        const runScriptRegexp = /(^|\W)(npm\s+run(?:-script)? (?:-s )?([^-][\w:\-]+)(?: -s)?)($|\W(?!-))/;
